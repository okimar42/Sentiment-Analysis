...
def create_test_analysis(
    ...
    **kwargs,
    ...
    return SentimentAnalysisFactory(
        query=query, source=source, model=model, status=status, **kwargs
    )
...
def create_complete_test_scenario(
    num_analyses: int = 3, results_per_analysis: int = 15
) -> Dict[str, Any]:
    """
...
def create_edge_case_test_data() -> Dict[str, Any]:
    ...
    empty_analysis = create_test_analysis(query="empty analysis", status="completed")
    ...
    # Failed analysis
    failed_analysis = create_test_analysis(query="failed analysis", status="failed")
...
def create_performance_test_data(
    num_analyses: int = 10, results_per_analysis: int = 100
) -> Dict[str, Any]:
    """
    ...
    with transaction.atomic():
        ...
        for i in range(num_analyses):
            analysis = create_test_analysis(
                query=f"performance test {i+1}", status="completed"
            )
            analyses.append(analysis)
...
def create_api_test_data() -> Dict[str, Any]:
    ...
    pending_analysis = create_test_analysis(query="pending API test", status="pending")

    processing_analysis = create_test_analysis(
        query="processing API test", status="processing"
    )

    completed_analysis = create_test_analysis(
        query="completed API test", status="completed"
    )

    # Add results to completed analysis
    api_results = create_test_results(completed_analysis, count=20, sentiment_mix=True)

    failed_analysis = create_test_analysis(query="failed API test", status="failed")
