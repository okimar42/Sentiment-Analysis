...
class PositiveSentimentResultFactory(SentimentResultFactory):
    ...
    score = factory.Faker('pyfloat', left_digits=0, right_digits=3, min_value=0.1, max_value=1.0)
...
class NegativeSentimentResultFactory(SentimentResultFactory):
    ...
    score = factory.Faker('pyfloat', left_digits=1, right_digits=3, min_value=-1.0, max_value=-0.1)
...
class NeutralSentimentResultFactory(SentimentResultFactory):
    ...
    score = factory.Faker('pyfloat', left_digits=0, right_digits=3, min_value=-0.05, max_value=0.05)
...
class HighBotProbabilityResultFactory(SentimentResultFactory):
    ...
    bot_probability = factory.Faker('pyfloat', left_digits=0, right_digits=2, min_value=0.7, max_value=1.0)
...
def create_analysis_with_results(num_results=10, **kwargs):
    ...
    results = SentimentResultFactory.create_batch(
        num_results, 
        analysis=analysis
    )
