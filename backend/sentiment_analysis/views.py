...
class SentimentAnalysisViewSet(viewsets.ModelViewSet):
    ...
    def update_result(self, request, pk=None, result_id=None):
        ...
        try:
            ...
            result = SentimentResult.objects.get(id=result_id, sentiment_analysis=analysis)
            ...
            if manual_sentiment is not None:
                ...
                result.override_by = request.user if request.user.is_authenticated else None
                result.save()
    ...
    def results(self, request: Any, pk: int = None) -> Response:
        ...
        if analysis.status != 'completed':
            return Response(
                {'status': analysis.status, 'message': 'Analysis is not completed yet.'}, 
    ...
    def summary(self, request: Any, pk: int = None) -> Response:
        ...
        analysis = get_object_or_404(SentimentAnalysis, pk=pk)
        results = SentimentResult.objects.filter(sentiment_analysis=analysis, is_ad=False).select_related('sentiment_analysis').prefetch_related('image_results')
        if not results.exists():
            return Response({'message': 'No results available yet'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def bot_analysis(self, request, pk=None):
        ...
        bot_analysis = {
            ...
            'avg_bot_probability': results.aggregate(avg=models.Avg('bot_probability'))['avg'] or 0
        }
            ...
            if 'twitter' in analysis.source:
                ...
                async def grok_summary_call():
                    ...
                    data = {
                        "model": "grok-beta",
                        "messages": [
                            {"role": "system", "content": "You are a social media sentiment analysis assistant. Summarize the overall sentiment and main opinions in a concise paragraph."},
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.post(url, headers=headers, json=data) as resp:
                            if resp.status != 200:
                                return f"Grok API error: {resp.status} {await resp.text()}"
    ...
    def export_csv(self, request, pk=None):
        ...
        analysis = get_object_or_404(SentimentAnalysis, pk=pk)
        if analysis.status != 'completed':
            return Response({'status': analysis.status, 'message': 'Analysis is not completed yet.'}, status=status.HTTP_202_ACCEPTED)
        results = analysis.results.filter(is_ad=False).select_related('sentiment_analysis').prefetch_related('image_results')
        if not results.exists():
            return Response({'message': 'No results available yet'}, status=status.HTTP_404_NOT_FOUND)
        # Define CSV header
        header = [
            'id', 'post_id', 'content', 'score', 'compound_score', 'created_at', 'has_images',
            'vader_score', 'gpt4_score', 'claude_score', 'gemini_score', 'grok_score',
            'sarcasm_score', 'is_sarcastic', 'perceived_iq', 'bot_probability', 'is_bot',
            'post_date', 'source_type'
        ]
        def row_gen():
            yield header
            for r in results:
                yield [
                    r.id, r.post_id, r.content, r.final_score, r.compound_score, r.created_at,
                    getattr(r, 'has_images', False),
                    getattr(r, 'vader_score', ''), getattr(r, 'gpt4_score', ''), getattr(r, 'claude_score', ''),
                    getattr(r, 'gemini_score', ''), getattr(r, 'grok_score', ''),
                    getattr(r, 'sarcasm_score', ''), getattr(r, 'is_sarcastic', ''), getattr(r, 'perceived_iq', ''),
                    getattr(r, 'bot_probability', ''), getattr(r, 'is_bot', ''),
                    r.post_date, r.source_type
                ]
