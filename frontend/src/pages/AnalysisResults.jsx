...
function generateSummaryText(summary, analysis) {
...
function hasTwitterSource(analysis) {
...
const AnalysisResults = ({ analysisId }) => {
  const { id } = useParams();
  ...
  const [sentimentByDate, setSentimentByDate] = useState([]);
  const [iqDistribution, setIQDistribution] = useState([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const sentimentData = data.sentiment_by_date.map(item => ({
      ...
      const iqData = data.iq_distribution.map(item => ({
  ...
  useEffect(() => {
    const timeout = setTimeout(() => setTimeoutReached(true), 2 * 60 * 1000); // 2 minutes
    ...
    return (
      <Container>
        <Typography color="error" variant="h6">
          Analysis is taking too long. Please try again later or contact support.
        </Typography>
      </Container>
    );
  }
